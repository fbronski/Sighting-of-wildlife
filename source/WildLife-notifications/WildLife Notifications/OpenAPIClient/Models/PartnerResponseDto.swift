//
// PartnerResponseDto.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

public struct PartnerResponseDto: Sendable, Codable, ParameterConvertible, Hashable {

    public var avatarColor: UserAvatarColor
    public var email: String
    public var id: String
    public var inTimeline: Bool?
    public var name: String
    public var profileChangedAt: Date
    public var profileImagePath: String

    public init(avatarColor: UserAvatarColor, email: String, id: String, inTimeline: Bool? = nil, name: String, profileChangedAt: Date, profileImagePath: String) {
        self.avatarColor = avatarColor
        self.email = email
        self.id = id
        self.inTimeline = inTimeline
        self.name = name
        self.profileChangedAt = profileChangedAt
        self.profileImagePath = profileImagePath
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case avatarColor
        case email
        case id
        case inTimeline
        case name
        case profileChangedAt
        case profileImagePath
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(avatarColor, forKey: .avatarColor)
        try container.encode(email, forKey: .email)
        try container.encode(id, forKey: .id)
        try container.encodeIfPresent(inTimeline, forKey: .inTimeline)
        try container.encode(name, forKey: .name)
        try container.encode(profileChangedAt, forKey: .profileChangedAt)
        try container.encode(profileImagePath, forKey: .profileImagePath)
    }
}


@available(iOS 13, tvOS 13, watchOS 6, macOS 10.15, *)
extension PartnerResponseDto: Identifiable {}
